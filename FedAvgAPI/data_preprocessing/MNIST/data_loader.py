...
def batch_data(data, batch_size):
    ...
    data_x = data['x']
    data_y = data['y']
    # logging.info("data_x: " + str(data_x))
    # logging.info('batched_x: ' + batched_x)
    ...
    for i in range(0, len(data_x), batch_size):
        ...
        # logging.info(np.array())

        # batched_x = torch.from_numpy(np.asarray(batched_x)).float()
        # batched_y = torch.from_numpy(np.asarray(batched_y)).long()
        batched_x = torch.from_numpy(np.array(batched_x,dtype=np.float)).float()
        batched_y = torch.from_numpy(np.array(batched_y)).long()

        # logging.info('batched_x: ' + str(batched_x[0]))
