...
DEFAULT_TRAIN_CLIENTS_NUM = 200
DEFAULT_TEST_CLIENTS_NUM = 200
DEFAULT_BATCH_SIZE = 20
DEFAULT_TRAIN_FILE = 'fasionMNIST_train_200_clients.h5'
DEFAULT_TEST_FILE = 'fasionMNIST_test_200_clients.h5'
...
def get_dataloader_noniid(dataset, data_dir, train_bs, test_bs, client_idx=None,train_data_f=DEFAULT_TRAIN_FILE):
    ...
    test_h5.close()
    return train_dl, test_dl
def load_partition_data_distributed_federated_fashionMNIST(process_id, dataset, data_dir,
...
def load_partition_data_federated_fashionMNIST(dataset, user_num, data_dir, batch_size=DEFAULT_BATCH_SIZE, train_data_f=DEFAULT_TRAIN_FILE,label_divided_num=10):
    # client ids
    logging.info("load_partition_data_federated_fashionMNIST")
    # logging.info("load_partition_data_federated_fashionMNIST")
    train_file_path = os.path.join(data_dir, train_data_f)
    test_file_path = os.path.join(data_dir, DEFAULT_TEST_FILE)
    with h5py.File(train_file_path, 'r') as train_h5, h5py.File(test_file_path, 'r') as test_h5:
